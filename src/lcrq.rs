...
use crate::queue::{EnqueueResult, QueueFull};
...
impl<T> CRQ<T> {
    ...
    pub fn new(len: usize) -> Self {
        let array: Box<[CachePadded<AtomicU128>]> = (0..len).map(|v| CachePadded::new(AtomicU128::new(u128::from(Node::<T>::new(v))))).collect();
        Self { 
